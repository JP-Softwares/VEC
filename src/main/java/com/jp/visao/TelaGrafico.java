...
public class TelaGrafico implements Initializable {
    @FXML
    ...
    public void initialize(URL url, ResourceBundle rb) {
        Run.telaGrafico = this;
    }

    @FXML
    private void handleMostrarGraficoMensal() {

        borderPane.setCenter(buildBarChartMensal());
    }

    @FXML
    private void handleMostrarGraficoPorTipo() {
        borderPane.setCenter(buildBarChartPorTipo());
    }

    public BarChart buildBarChartMensal() {
        CategoryAxis xAxis = new CategoryAxis();
        xAxis.setLabel("Mês do Ano " + ano); // Mensal

        NumberAxis yAxis = new NumberAxis();
        yAxis.setLabel("Gastos (R$) - " + ano); // Valor

        BarChart barChart = new BarChart(xAxis, yAxis);

        XYChart.Series dataSeries1 = new XYChart.Series();
        dataSeries1.setName("Gráfico de todos os gastos agrupados por meses do ano");

        double[] gastos = null;

        try{
            gastos = Run.gastosControle.valorTotalMes(veiculo, ano);
        }catch (Exception e){
            e.printStackTrace();
        }

        dataSeries1.getData().add(new XYChart.Data("Janeiro", gastos[0]));
        dataSeries1.getData().add(new XYChart.Data("Fevereiro", gastos[1]));
        dataSeries1.getData().add(new XYChart.Data("Março", gastos[2]));
        dataSeries1.getData().add(new XYChart.Data("Abril", gastos[3]));
        dataSeries1.getData().add(new XYChart.Data("Maio", gastos[4]));
        dataSeries1.getData().add(new XYChart.Data("Junho", gastos[5]));
        dataSeries1.getData().add(new XYChart.Data("Julho", gastos[6]));
        dataSeries1.getData().add(new XYChart.Data("Agosto", gastos[7]));
        dataSeries1.getData().add(new XYChart.Data("Setembro", gastos[8]));
        dataSeries1.getData().add(new XYChart.Data("Outubro", gastos[9]));
        dataSeries1.getData().add(new XYChart.Data("Novembro", gastos[10]));
        dataSeries1.getData().add(new XYChart.Data("Dezembro", gastos[11]));
    ...
    public BarChart buildBarChartPorTipo() {
        CategoryAxis xAxis = new CategoryAxis();
        xAxis.setLabel("Tipos de Gasto"); // Mensal
        ...
        XYChart.Series dataSeries1 = new XYChart.Series();
        dataSeries1.setName("Gráfico de todos os gastos agrupados por tipo de gasto");

        HashMap<String, Double> tipoDeGastos = null;

        try{
            tipoDeGastos = Run.gastosControle.valorTotalTipo(veiculo, ano);
        }catch (Exception e){
            e.printStackTrace();
        }
        HashMap<String, Double> finalTipoDeGastos = tipoDeGastos;
        tipoDeGastos.keySet().forEach(s -> {
            dataSeries1.getData().add(new XYChart.Data(s, finalTipoDeGastos.get(s)));
        });
